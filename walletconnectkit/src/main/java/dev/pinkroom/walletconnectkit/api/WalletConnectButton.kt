...
class WalletConnectButton @JvmOverloads constructor(
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (layoutParams.width == WRAP_CONTENT) {
            width = MeasureSpec.makeMeasureSpec(400.px, MeasureSpec.EXACTLY)
        }
        if (layoutParams.height == WRAP_CONTENT) {
            height = MeasureSpec.makeMeasureSpec(100.px, MeasureSpec.EXACTLY)
        }
    ...
    private fun initSrc(attrs: AttributeSet?) {
        ...
        if (src == -1) setImageResource(R.drawable.button)
        typedArray.recycle()
    }
